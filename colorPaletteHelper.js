const buildPalette = (colorsList) => {
    ...
    const hexColorComplementary = new Set()
    for (let i = 0; i < orderedByColor.length; i++) {
        ...
        hexColors.add(rgbToHex(orderedByColor[i]))

        if(hslColors[i]){
            hexColorComplementary.add(hslToHex(hslColors[i]))
        }
    }

    return {
        colors: [...hexColors],
        complementary: [...hexColorComplementary]
    }
}
